  ...
  app.get('/narratives', async (request: FastifyRequest) => {
    ...
    return {
      ...
      data: {
        ...
        window,
        interpretation: formattedNarratives.length === 0
          ? {
              headline: 'No market narratives detected',
              description: `We analyzed ${allSignals.length} signals across ${topTokens.length} tokens. No coordinated patterns found.`,
            }
          : {
              headline: `${formattedNarratives.length} market narrative${formattedNarratives.length > 1 ? 's' : ''} identified`,
